...
router.post('/api/user/warranty', async(req, res) => {
    const { email, couponCodeToSearch } = req.body; // 'couponCodeToSearch' es como el ID de la reserva, guardada dentro de user.classes[] en MongoDB
    // const user = await User.findById({_id: userId});
    const user = await User.findOne({email: email});
    console.log('/api/user/warranty, user finded:');
    console.log(user);
    console.log(`The coupon code was ${couponCodeToSearch}`);
    if( couponCodeToSearch == process.env.HIPNOCENTRO_WARRANTY_CODE ) 
        res.send({error: false, result: '¡Genial! Acabas de utilizar el código comodín.'});
    else if(user) {
        ...
        res.send({error: true, result: 'No existe este código de reserva o persona.'});
});
